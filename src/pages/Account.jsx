  ...
  return (
    <div className="space-y-6 max-w-lg mx-auto">
      ...
      <div className="border border-border p-5 space-y-3 rounded-[50px] bg-[hsl(var(--popover))]">
        <div className="flex items-center justify-between rounded-[50px] bg-[hsl(var(--popover))] border-2 border-[hsl(var(--background))]">
          ...
          {showSettings && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="px-5 pb-5 space-y-4 border-t border-border pt-3">
                ...
                {/* Custom rest toggle */}
                <div className="flex items-center justify-between px-4 py-3 bg-[hsl(var(--popover))] rounded-[50px] border-2 border-[hsl(var(--background))]">
                  ...
                  <button
                    ...
                    className={`relative w-12 h-6 rounded-full transition-colors shrink-0 ${customRestEnabled ? "bg-primary" : "bg-[hsl(var(--background))]"}`}>
                  ...
                  <div className="bg-[hsl(var(--background))] rounded-2xl px-4 py-3 space-y-2">
                    ...
                    {editingRest && (
                      <div className="flex gap-2 items-center">
                        <Input type="number" value={defaultRestSeconds} onChange={(e) => setDefaultRestSeconds(e.target.value)} placeholder="es. 90" className="h-9 rounded-xl flex-1" min={10} />
                        <span className="text-sm text-muted-foreground shrink-0">secondi</span>
                        <Button size="sm" onClick={saveRestDefault} className="rounded-xl h-9 px-4"><Check className="w-3.5 h-3.5" /></Button>
                        <Button size="sm" variant="outline" onClick={() => setEditingRest(false)} className="rounded-xl h-9 px-3"><X className="w-3.5 h-3.5" /></Button>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex items-center justify-between px-4 py-3 bg-[hsl(var(--popover))] rounded-[50px] border-2 border-[hsl(var(--background))]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium">RPE per serie</p>
                      <p className="text-xs text-muted-foreground mt-0.5">Registra la difficoltà percepita (1-10) per ogni serie</p>
                    </div>
                  </div>
                  <button
                    onClick={() => {
                      const newVal = !rpePerSetEnabled;
                      setRpePerSetEnabled(newVal);
                      localStorage.setItem("rpePerSetEnabled", String(newVal));
                      toast.success(newVal ? "RPE per serie attivato" : "RPE per serie disattivato");
                    }}
                    className={`relative w-12 h-6 rounded-full transition-colors shrink-0 ${rpePerSetEnabled ? "bg-primary" : "bg-[hsl(var(--background))]"}`}>
                    <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${rpePerSetEnabled ? "left-7" : "left-1"}`} />
                  </button>
                </div>
                    ...
                    <div key={key} className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`relative w-10 h-5 rounded-full transition-colors shrink-0 ${notifSettings[key].enabled ? "bg-primary" : "bg-[hsl(var(--background))]"}`}>
